    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Raport</h2>
                <a href="{{ route('students.show', $report->siswa_id) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition text-sm">Kembali</a>
            </div>
            ...
            <!-- Tabs Navigation (Disabled in edit to prevent switching category type) -->
            <div class="flex space-x-4 mb-8 border-b border-gray-200 dark:border-gray-700">
                <div class="py-2 px-4 border-b-2 border-blue-500 text-blue-600 font-medium text-sm">
                    {{ $report->category->name === 'Probing' ? 'Probing (Aktivitas)' : 'Akademik/Diniyah/Ibadah' }}
                </div>
            </div>
            ...
            <form action="{{ route('admin.reports.update', $report) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-100 dark:border-gray-800">
                    <div>
                        ...
                        <div class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-700 dark:text-gray-300">
                    ...
                    <div>
                        ...
                        <input type="text" name="period" value="{{ old('period', $report->period) }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                </div>
                ...
                <div x-show="activeTab === 'akademik'" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        <h3 class="text-lg font-bold text-gray-800 dark:text-gray-200 border-l-4 border-blue-500 pl-3">Edit Nilai Subjek</h3>
                        <div class="grid grid-cols-1 gap-4">
                            @foreach($report->category->subjects as $subject)
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-4 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-sm">
                                    <div class="font-medium text-gray-700 dark:text-gray-300">{{ $subject->name }}</div>
                                    <div>
                                        <input type="number" name="grades[{{ $subject->id }}][score]" value="{{ $grade->score ?? '' }}" step="0.01" min="0" max="100" placeholder="Nilai (0-100)" class="w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white text-sm">
                                    </div>
                                    <div>
                                        <input type="text" name="grades[{{ $subject->id }}][description]" value="{{ $grade->description ?? '' }}" placeholder="Keterangan / Catatan" class="w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white text-sm">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <template x-for="(activity, index) in activities" :key="activity.id">
                            <div class="p-6 border border-gray-200 dark:border-gray-700 rounded-2xl bg-gray-50 dark:bg-gray-900/30 relative shadow-sm">
                                <button type="button" @click="activities = activities.filter(a => a.id !== activity.id)" class="absolute top-4 right-4 text-red-400 hover:text-red-600 transition">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="md:col-span-1">
                                        ...
                                        <input type="text" :name="`probing[${index}][name]`" :value="activity.name" required class="w-full rounded-lg border-gray-300 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                                    </div>
                                    <div class="md:col-span-1">
                                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Ganti Foto (Kosongkan jika tidak ingin ganti)</label>
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <textarea :name="`probing[${index}][description]`" x-text="activity.desc" rows="3" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"></textarea>
                                    </div>
                                </div>
                            </div>
                        </template>

                        <button type="button" @click="activities.push({ id: Date.now(), name: '', desc: '', img: '' })" class="w-full py-4 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl text-gray-500 hover:text-blue-600 flex items-center justify-center gap-2 font-medium transition">
                ...
                <div class="mt-10">
                    ...
                    <textarea name="summary_notes" rows="4" class="w-full rounded-xl border-gray-300 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">{{ old('summary_notes', $report->summary_notes) }}</textarea>
                </div>

                <div class="mt-10 pt-6 border-t border-gray-100 dark:border-gray-700 flex justify-between">
                    <button type="button" onclick="if(confirm('Hapus raport ini?')) document.getElementById('delete-form').submit();" class="text-red-600 hover:text-red-800 font-semibold transition">
                        Hapus Raport
                    </button>
