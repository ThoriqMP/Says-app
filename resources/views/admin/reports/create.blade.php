        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-[20px] border border-gray-200 dark:border-gray-700 p-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mt-1">Menginput laporan untuk periode tertentu secara spesifik.</p>
                </div>
                <a href="{{ route('students.show', $selectedStudentId) }}" class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-5 py-2.5 rounded-xl font-semibold hover:bg-gray-200 transition">
                    Kembali ke Profil
                </a>
            </div>
            ...
            <form action="{{ route('admin.reports.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            @if($selectedCategory)
                                @foreach($selectedCategory->subjects as $subject)
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-5 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm hover:border-blue-200 transition">
                                        ...
                                        <div>
                                            <input type="number" name="grades[{{ $subject->id }}][score]" step="0.01" min="0" max="100" placeholder="Nilai (0-100)" class="w-full rounded-xl border-gray-300 dark:bg-gray-700 dark:text-white text-sm font-bold">
                                        </div>
                                        <div>
                                            <input type="text" name="grades[{{ $subject->id }}][description]" placeholder="Keterangan / Catatan" class="w-full rounded-xl border-gray-300 dark:bg-gray-700 dark:text-white text-sm">
                                        </div>
                                    </div>
                                @endforeach
                            @endif
                ...
                <!-- Probing Form -->
                <template x-if="isProbing">
                    <div x-data="{ activities: [{ id: Date.now(), name: '', desc: '' }] }">
                                ...
                                <div class="p-8 border border-gray-200 dark:border-gray-700 rounded-[24px] bg-gray-50 dark:bg-gray-900/30 relative shadow-sm">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div>
                                            ...
                                            <input type="text" :name="`probing[${index}][name]`" required class="w-full rounded-xl border-gray-300 focus:ring-blue-500 dark:bg-gray-700 dark:text-white font-bold">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Upload Foto Aktivitas</label>
                                        ...
                                        <div class="md:col-span-2">
                                            ...
                                            <textarea :name="`probing[${index}][description]`" rows="4" class="w-full rounded-xl border-gray-300 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" placeholder="Ceritakan detail aktivitas siswa di sini..."></textarea>
                                        </div>
                                    </div>
                                </div>
                            </template>

                            <button type="button" @click="activities.push({ id: Date.now(), name: '', desc: '' })" class="w-full py-6 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-[24px] text-gray-500 hover:text-blue-600 hover:border-blue-400 hover:bg-blue-50 transition flex items-center justify-center gap-2 font-bold text-lg">
                ...
                <div class="mt-12">
                    ...
                    <textarea name="summary_notes" rows="4" class="w-full rounded-2xl border-gray-300 focus:ring-blue-500 dark:bg-gray-700 dark:text-white p-5" placeholder="Berikan catatan kesimpulan perkembangan siswa selama periode ini..."></textarea>
                </div>
