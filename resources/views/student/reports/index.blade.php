...
<div class="py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6">
        @forelse($reports as $report)
            <div class="bg-white dark:bg-gray-800 rounded-[24px] md:rounded-[32px] border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden">
                <div class="p-4 md:p-8">
                    <div class="flex justify-between items-start mb-4 md:mb-6">
                        ...
                        <span class="px-2 md:px-4 py-1 md:py-1.5 bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest">{{ $report->created_at->format('M Y') }}</span>
                    </div>
                    
                    <h3 class="text-sm md:text-2xl font-black text-gray-900 dark:text-white mb-1 md:mb-2 line-clamp-1">{{ $report->category->name }}</h3>
                    <p class="text-gray-500 dark:text-gray-400 font-bold text-[10px] md:text-sm mb-4 line-clamp-1">{{ $report->period }}</p>

                    <div class="flex items-center gap-2 mb-4 md:mb-8 p-2 md:p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[9px] md:text-xs font-bold text-gray-700 dark:text-gray-300 truncate">{{ $report->teacher->name ?? 'Admin Sekolah' }}</p>
                        </div>
                    </div>
                    
                    <div class="space-y-2 md:space-y-3">
                        <a href="{{ route('student.reports.show', $report) }}" class="block w-full py-2.5 md:py-4 bg-gray-900 dark:bg-gray-700 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm text-center hover:bg-blue-600 transition-colors duration-300 active:scale-95">
                            Lihat Detail
                        </a>
                        <a href="{{ route('student.reports.pdf', $report) }}" class="flex items-center justify-center gap-1.5 md:gap-2 w-full py-2.5 md:py-4 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm text-center hover:bg-blue-100 transition-colors duration-300 active:scale-95">
